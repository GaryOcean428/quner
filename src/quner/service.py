...
def LAUNCHER_PATH() -> str:
    """A root-visible launcher path, so `sudo quner …` works after install even
    when quner was pipx-installed into a user's ~/.local/bin."""
    return os.environ.get("QUNER_LAUNCHER_PATH", "/usr/local/bin/quner")


_LAUNCHER_MARKER = "# quner-launcher"
...
def _write_launcher() -> str | None:
    """Drop a root-visible shim at LAUNCHER_PATH so subsequent `sudo quner …`
    resolves. Never clobbers an existing file (e.g. a system pip install)."""
    path = LAUNCHER_PATH()
    if os.path.exists(path):
        return None
    shim = (f"#!/bin/sh\n{_LAUNCHER_MARKER} (auto-generated by `quner install`; "
            f"removed by `quner uninstall`)\n"
            f'exec "{sys.executable}" -m quner "$@"\n')
    try:
        ...
        return None  # e.g. non-root / read-only /usr/local — harmless


def _remove_launcher() -> None:
    """Remove the launcher, but only if it is ours (marker present)."""
    path = LAUNCHER_PATH()
    try:
        with open(path) as fh:
            if _LAUNCHER_MARKER not in fh.read():
                return
        os.remove(path)
    except OSError:
        pass
...
def install(dry_run: bool = False, interval: str = "30min") -> list[str]:
    ...
    if dry_run:
        ...
        for name, text in units.items():
            print(f"\n# --- {name} ---\n{text}")
        print(f"# would drop a root-visible launcher at {LAUNCHER_PATH()} "
              "(if absent) so `sudo quner …` resolves")
    ...
    if launcher:
        print(f"# dropped root-visible launcher: {launcher}")
